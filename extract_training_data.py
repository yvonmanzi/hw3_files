...
class FeatureExtractor(object):
    ...
    def __init__(self, word_vocab_file, pos_vocab_file):
        ...
        #? what's the point of having this one?    
    ...
    def get_input_representation(self, words:list, pos, state:State):
        ...
        # print("stack", stack)
        indices = []
        null_index = self.word_vocab.get('<NULL>')
        ukn_index = self.word_vocab.get('<UKN>')
        cd_index = self.word_vocab.get('<CD>')
        nnp_index = self.word_vocab.get('<NNP>')
        root_index = self.word_vocab.get('<ROOT>')

        def check_pos(word_id):
                # if we get zero from the stack, means we have root. 
                if not word_id:
                    indices.append(root_index)
                    return 
                
                partofspeech = pos[word_id]

                if partofspeech == 'NNP':
                    indices.append(nnp_index)
                    return 
                if partofspeech == 'CD':
                    indices.append(cd_index)
                    return 
                embedding_index = self.word_vocab.get(words[word_id].lower(), None)
                indices.append(embedding_index) if embedding_index else indices.append(ukn_index)
        #Words from the stack
        for word_id in stack[::-1][:3]:
            check_pos(word_id)
